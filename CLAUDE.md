...
the four quiz ladders they play for fun.
...
- **The four quiz ladders are not curriculum**, and each says so at the top of
  its own bank. They are on the Quiz tab, one panel each, built by `climbPanel()`
  out of the `LADDERS` array in `training.js`:
  ...
  topics — MOE science starts in P3, so there is no sheet this could come off)
  and `zh` (`ZH_LADDER`, hear the word and tap the characters).
  ...
  up, and the score is the rung cleared. The rung a run starts on is picked on
  the panel and sticks per ladder on the device (`qfrom:<id>`, not synced);
  rungs below it are never asked, so they never count as cleared — the ladder
  ...
  he needs to drill. None are split by boy either: the panel shows the best
